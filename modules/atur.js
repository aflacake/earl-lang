...
function atur(tokens) {
    ...
    if (!namaAtribut) {
        console.error(`Format target '${path}' tidak valid. Gunakan :var: atau Kelas.attr`);
        return;
    }

    const instance = memory[namaInstance];
    if (!instance || instance.__tipe !== 'kelas') {
        console.error(`Instance '${namaInstance}' tidak ditemukan atau bukan kelas.`);
        return; 
    }
